...
public class CreditCardUtils
{
  ...
  public static boolean isExpirationDateValid(String date)
  {
    ...
    MySystem.variable("credit, real = [" + cMonth + "/" + cYear + "], [" + month + "/" + year + "]");
  ...
  public static boolean isCreditCardNumberValid(String number, String[] allowedCreditCardTypes)
  {
    ...
    switch (firstChar)
    {
      ...
      case '3' :
        return (number.length() == 15) && StringUtils.isIn(CreditCardHolder.AMERICAN_EXPRESS, allowedCreditCardTypes);
      default :
        return false;
    }
  ...
  public static boolean isCardSecurityCodeValid(String cardSecurityCode, String creditCard)
  {
    ...
    MySystem.variable(String.format("security code: %s, cc#: %s", cardSecurityCode, firstChar));
    switch (firstChar)
    {
      case '4' :
  ...
  public static String constructVerifiedCreditCardNumber(String number)
  {
    ...
    if (CreditCardUtils.isCreditCardNumberValid(ver, CreditCardHolder.ALL))
    {
      ...
      switch (firstChar)
      {
        ...
        case '5' :
        case '6':
          ver = ver.substring(0, 4) + "-" + ver.substring(4, 8) + "-" + ver.substring(8, 12) + "-" + ver.substring(12, 16);
          break;
        case '3' :
  ...
  /***********************************************************************/
  public static TransactionResponse processCreditCard(CreditCardConfiguration configuration, CreditCardParameters params)
  {
    return processCreditCard(configuration.getHostAddress(), configuration.getHostPort(), configuration.getCertificatePath(), params);
  }
  /***********************************************************************/
  public static TransactionResponse processCreditCard(String hostAddress, int hostPort, String certificatePath, CreditCardParameters params)
  {
